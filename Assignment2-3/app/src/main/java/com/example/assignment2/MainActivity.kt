...
import com.example.assignment2.vm.CourseViewModelFactory
import androidx.compose.runtime.collectAsState

class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val factory = CourseViewModelFactory.getInstance(this)
        setContent {
            val vm: CourseViewModel = viewModel(factory = factory)
            MaterialTheme { MinimalCourseApp(vm) }
        }
    }
}

@Composable
fun MinimalCourseApp(vm: CourseViewModel) {
    // Observable state from the ViewModel
    val courses = vm.courses.collectAsState().value
    val selectedCourseId = vm.selectedId
